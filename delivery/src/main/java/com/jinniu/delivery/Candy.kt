...
//        val requestOptions = if (transform != null) {
//            RequestOptions.bitmapTransform(MultiTransformation(transform))
//        } else centerCrop
...
fun Any.resDrawable(drawableRes: Int, scale: Float = 1F): Drawable =
        ContextCompat.getDrawable(MyApplication.context, drawableRes)!!.apply {
            setBounds(0, 0, (minimumWidth * scale).toInt(), (minimumHeight * scale).toInt())
        }

fun Any.resString(stringRes: Int): String = MyApplication.context.getString(stringRes)
fun Any.resDimension(dimensionRes: Int): Int = MyApplication.context.resources.getDimensionPixelSize(dimensionRes)
...
fun Any.statusBarHeight(): Int {
    ...
    try {
        ...
        statusHeight = MyApplication.context.resources.getDimensionPixelSize(height)
    } catch (e: Exception) {
        e.printStackTrace()
    }
    return statusHeight
...
fun View.anim(animRes: Int, visible: Int = View.VISIBLE) {
    if (visibility != visible) {
        visibility = visible
        startAnimation(AnimationUtils.loadAnimation(MyApplication.context, animRes))
    }
}